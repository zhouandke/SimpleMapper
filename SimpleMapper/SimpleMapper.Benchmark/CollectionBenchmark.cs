...
namespace SimpleMapper.Benchmark
{
    public class CollectionBenchmark
    {
        ...
        public void CollectionMapping_AutoMapper()
        {
            ...
            {
                autoMapper.Map<TargetWithCollections>(_source);
            }
        }
        ...
        [Benchmark]
        public void CollectionMapping_SimpleMapperAsShallowCopy()
        {
            for (int i = 0; i < Iterations; i++)
            {
                var target = ZK.Mapper.SimpleMapper.Default.Map<TargetWithCollections>(_source);
            }
        }

        [Benchmark]
        public void CollectionMapping_SimpleMapperAsDeepCopy()
        {
            for (int i = 0; i < Iterations; i++)
            {
                var target = ZK.Mapper.SimpleMapper.Default.MapAsDeepCopy<TargetWithCollections>(_source);
            }
        }
